...
func (p *productMYSQLRepository) FindByID(ctx context.Context, id string) (*models.Product, error) {
	...
	err := p.db.QueryRowContext(ctx, "SELECT product_id, product_name, product_price, product_amount, safety_stock_amount FROM products WHERE product_id = ?", id).Scan(&product.ProductID, &product.Name, &product.Price, &product.Amount, &product.QuantityOfSafetyStock)
...
func (p *productMYSQLRepository) FindByName(ctx context.Context, name string) (*models.Product, error) {
	var product models.Product
	err := p.db.QueryRowContext(ctx, "SELECT * FROM products WHERE product_name = ?", name).Scan(&product.ProductID, &product.Name, &product.Price, &product.Amount, &product.QuantityOfSafetyStock)
	if err != sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
// GetAllProducts implements repositories.ProductRepository.
func (p *productMYSQLRepository) GetAllProducts(ctx context.Context) ([]models.Product, error) {
